...
>>>>>>> Working lighting implementation
	...
	function castLightsToObject(light, box) {
		if (collideSphere(box, light)) {
			...
			c = 8, // cause we dont work with polygons
				_wallNormals,
				_lightVector = List.get(8, "f32"),
				facing = 0,
				lastIndex = 0,
				lastPoint = List.get(2, "f32"),
				lastPointIndex = NaN,
				firstPoint = List.get(2, "f32"),
				firstPointIndex = NaN;
			var boxVertices = Physics.getVertices(box, TRUE);
			...
			if (lastPointIndex != firstPointIndex) {
				...
				var shadowFar = List.get(8, "f32");
				var shadowNear = List.get(8, "f32");
				var increment = 0;
			...
			List.put(lastPoint);
			List.put(firstPoint);
			List.put(boxVertices);
			List.put(_lightVector);
			List.put(shadowNear);
			List.put(shadowFar);
		}
	...
	function updateObjectsInner(box, context) {
		...
		Draw.poly(box, context, color, color);
	}
	...
	function start() {
		...
		Control.on("change", controlInner);
	...
	function setup() {
		...
		Config.action(LIGHTING_X, LIGHTING_Y); // two movement planes are needed since thats how the controls handle the axes
		Config.input(MOUSE, 0);
		Config.bind(MOUSE, 0, LIGHTING_X, MOUSE_X);
		Config.bind(MOUSE, 0, LIGHTING_Y, MOUSE_Y);
		Control.listen(document, MOUSE);

		_lightId = Struct.make(13, "f32");
		_lightStruct = Struct.get(_lightId);

		_boxId = Struct.make(11, "f32");
		_boxStruct = Struct.get(_boxId);
	}
	// end functions
